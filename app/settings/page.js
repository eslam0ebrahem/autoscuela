...
function SettingsContent() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 animate-fade-in pb-12 px-4">
      ...
      {/* Subscription Hero */}
      <div
        className={`card border-0 overflow-hidden relative ${user?.isPremium ? 'bg-gradient-to-br from-indigo-600 via-indigo-500 to-emerald-600 text-white shadow-2xl' : 'bg-slate-100 dark:bg-slate-800'}`}
      >
        {user?.isPremium && (
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="text-center md:text-left p-6">
            ...
            <h2 className="text-2xl font-black">
              {user?.isPremium ? 'Vialia Premium' : 'Vialia Free'}
            </h2>
            <p className="mt-2 text-sm font-medium opacity-80 max-w-sm">
              {user?.isPremium
                ? t(
                    'Disfrutas de acceso ilimitado a IA y exámenes.',
                    'You have full access to AI and exams.'
                  )
                : t(
                    'Suscríbete para desbloquear el análisis IA y exámenes ilimitados.',
                    'Subscribe to unlock AI analysis and unlimited exams.'
                  )}
            </p>
          </div>
          {!user?.isPremium && (
        ...
        <div className="card glass space-y-4">
          ...
          <form onSubmit={handleSaveProfile} className="space-y-4">
            <div className="space-y-1">
              <label className="text-[10px] font-black uppercase tracking-widest text-ink-light ml-1">
                {t('Nickname', 'Nickname')}
              </label>
              <input
                value={nickname}
            ...
            <button
              ...
              className="btn-primary w-full py-4 rounded-2xl font-black shadow-lg shadow-primary/20"
            >
      ...
      <div className="card glass">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="text-center">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-ink-light">
              XP Total
            </p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-black text-orange-500">
              {user?.gamification?.maxStreak || 0}
            </p>
            <p className="text-[10px] font-black uppercase tracking-widest text-ink-light">
              Max Streak
            </p>
          ...
          <div className="text-center">
            <p className="text-2xl font-black text-purple-500">
              {user?.gamification?.earnedBadges?.length || 0}
            </p>
            <p className="text-[10px] font-black uppercase tracking-widest text-ink-light">
              Badges
            </p>
