...
export const ExamGenerateSchema = z.object({
  ...
  topic_filter: z
    .union([z.string(), z.array(z.string()).max(20, 'Maximum 20 topic filters allowed')])
    .optional()
    .nullable(),
  ...
  num_questions: z
    ...
    .default(30)
    .optional(),
  only_new_questions: z.boolean().default(false).optional(),
  source: z.string().default('standard').optional(),
  timer_minutes: z.number().int().min(0).max(180).optional(),
})
...
export const QuestionReportSchema = z.object({
  questionId: ObjectIdSchema,
  reason: z.enum(['wrong_answer', 'unclear_text', 'outdated', 'missing_image', 'other']),
...
export const AIStudyPlanSchema = z.object({
  targetDate: z
    ...
    .refine((v) => new Date(v) > new Date(), 'Target date must be in the future'),
