...
export const BADGES = [
  ...
  {
    id: 'half_century_streak',
    name: { es: 'Medio Siglo', en: 'Half Century Streak' },
    description: { es: '50 dias de racha consecutivos', en: '50-day study streak' },
    icon: '🌋',
    color: '#B91C1C',
  },
  {
    id: 'elite_scholar',
    name: { es: 'Erudito de Elite', en: 'Elite Scholar' },
    description: { es: 'Gana 5000 XP', en: 'Earn 5000 XP' },
    icon: '👑',
    color: '#4C1D95',
  },
]
...
export function checkBadgeConditions(user, examSession, dailyAnswerCount, options = {}) {
  ...
  // Half Century Streak: 50-day streak
  if (!earned.includes('half_century_streak') && streak >= 50) {
    newBadges.push('half_century_streak')
  }
  ...
  // Speed Demon: official exam in under 15 minutes (900 seconds) and passed
  if (
  ...
  // Centurion: 1000 total questions answered
  if (!earned.includes('centurion') && totalAnswered >= 1000) {
    newBadges.push('centurion')
  }

  // Speed Learner: average < 20 sec/question on official exam and passed
  if (
  ...
  // Elite Scholar: 5000 total XP
  if (!earned.includes('elite_scholar') && totalXP >= 5000) {
    newBadges.push('elite_scholar')
  }
...
// Throttle rank updates per user to max once per 5 minutes
const rankUpdateThrottle = new Map()
...
export async function updateLeaderboardRank(userId) {
  try {
    const now = Date.now()
    const lastUpdate = rankUpdateThrottle.get(userId)
    if (lastUpdate && now - lastUpdate < 5 * 60 * 1000) {
      return // Skip if updated in the last 5 minutes
    }
    rankUpdateThrottle.set(userId, now)
