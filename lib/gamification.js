...
export const BADGES = [
  ...
  },
  {
    ...
    description: { es: 'Corrige 50 errores del banco de errores', en: 'Correct 50 mistakes from mistake bank' },
    icon: '🎯',
    color: '#DC2626',
  },
  {
    ...
    description: { es: 'Alcanza 90%+ en un tema con 50+ preguntas', en: 'Reach 90%+ on a topic with 50+ questions' },
    icon: '👑',
    color: '#F59E0B',
  },
  {
    ...
    description: { es: 'Responde preguntas de todos los temas', en: 'Answer questions from every topic' },
    icon: '📖',
    color: '#6366F1',
  },
  {
  ...
  },
  {
    ...
    description: { es: 'Promedio menos de 20 seg/pregunta en examen oficial', en: 'Average under 20 sec/question on official exam' },
    icon: '🚀',
    color: '#0EA5E9',
  },
  {
...
export function checkBadgeConditions(user, examSession, dailyAnswerCount, options = {}) {
  ...
  if (!earned.includes('speed_demon') && examSession?.mode === 'official' && examSession?.totalTimeTakenSeconds < 900) {
  ...
  if (!earned.includes('topic_master') && topicAccuracies.some((t) => t.accuracy >= 90 && t.attempted >= 50)) {
    newBadges.push('topic_master')
  }

  // Encyclopedia: answered from all topics
  if (!earned.includes('all_topics') && uniqueTopicsAnswered > 0 && uniqueTopicsAnswered === totalTopics && totalTopics >= 10) {
  ...
  if (!earned.includes('speed_learner') && examSession?.mode === 'official' && avgTimePerQuestion > 0 && avgTimePerQuestion < 20) {
