...
async function callGroq({ messages, model = MODEL, temperature = TEMPERATURE, json = true }) {
  let lastError
...
function buildInsightsFallback(lang) {
  const es = lang === 'es'
  return {
    readiness_score: null, weak_topics: [],
    coach_message: es
    ...
    predicted_ready_date: null, improvement_rate: null,
    study_tips: [], topic_priority_order: [], _fallback: true,
  }
}
...
export async function getQuestionExplanation({ question, options, correctIdx, selectedIdx, helpHtml, lang }) {
  const fallback = {
...
export async function getSmartHint({ question, options, correctIdx, lang }) {
  ...
  const fallback = {
    hint: lang === 'es' ? 'Piensa en las normas de tráfico generales.' : 'Think about general traffic rules.',
...
export async function getExamCoachFeedback({ examSummary, lang }) {
  const fallback = {
    ...
    summary: lang === 'es' ? 'Completa más exámenes para obtener análisis personalizados.' : 'Complete more exams for personalized analysis.',
    strengths: [], weaknesses: [],
    next_step: lang === 'es' ? 'Practica los temas con más errores.' : 'Practice topics with the most errors.',
    priority_topics: [],
    confidence_boost: lang === 'es' ? '¡Cada intento te acerca más al aprobado!' : 'Every attempt brings you closer to passing!',
    _fallback: true,
  }
  try {
    const raw = await callGroq({
...
export async function getExamRecommendation({ recentStats, lang }) {
  const fallback = {
    ...
    reason: lang === 'es' ? 'Practica con el formato oficial para acostumbrarte al examen real.' : 'Practice with the official format to get used to the real exam.',
    suggested_topics: [],
    suggested_question_count: 30,
    urgency: 'medium',
    tip: lang === 'es' ? 'Estudia 20 minutos al día para mejorar consistentemente.' : 'Study 20 minutes daily for consistent improvement.',
    _fallback: true,
  }
