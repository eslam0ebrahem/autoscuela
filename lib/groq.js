...
export async function getExamCoachFeedback({ examSummary, sessionHistory = [], lang }) {
  const fallback = {
      ...
      messages: [
        {
          ...
          content:
            ...
            (sessionHistory.length > 0
              ? '\n\nAlso include "trend": "improving"|"stable"|"declining", "improvement_pct": score change vs oldest session (number or null), "consistency_score": 0-100.'
              : ''),
        },
        {
          role: 'user',
...
export async function getStudyPlan({ skillProfile, targetDate, dailyMinutes = 30, lang = 'es' }) {
  ...
  try {
    ...
    const prompt = `You are a DGT Spanish driving exam coach. Create a personalized study plan.
...
- Weak topics (Spanish tag + accuracy): ${weakTopics || 'none identified yet'}
...
Language: ${lang === 'es' ? 'Spanish' : 'English'}. Be encouraging and specific. Maximum ${Math.min(weeksAvailable, 8)} weeks.
Return ONLY valid JSON.`
...
export async function getMistakePatterns({ mistakeGroups, lang = 'es' }) {
  ...
  try {
    ...
    const prompt = `You are a DGT Spanish driving exam expert. Analyze these mistake patterns and identify conceptual knowledge gaps.
...
Language: ${lang === 'es' ? 'Spanish' : 'English'}. Be direct and specific — no generic advice.
Return ONLY valid JSON.`
