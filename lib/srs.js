...
export function calculateSRS({ easinessFactor = 2.5, interval = 1, repetitions = 0 } = {}, grade) {
  ...
  if (grade < 3) {
    // Failed — penalize interval rather than full reset, acknowledging partial retention
    newRepetitions = Math.max(0, repetitions - 1)
    newInterval = Math.max(1, Math.round(interval * 0.5))
  } else {
    // Successful review
    newRepetitions = repetitions + 1
    // Only use hardcoded early intervals if we're actually at the beginning of learning
    if (newRepetitions === 1 && interval <= 1) newInterval = 1
    else if (newRepetitions === 2 && interval <= 1) newInterval = 6
    else newInterval = Math.round(Math.max(interval, 1) * newEF)
  }
