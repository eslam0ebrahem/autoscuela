...
function getConsistencyScore(studyTrends = []) {
  if (!studyTrends.length) return null
  const activeDays = studyTrends.filter((d) => (d.questions || 0) > 0).length
  return Math.round((activeDays / studyTrends.length) * 100)
}
...
function computeReadinessScore({ aggregatedData, studyTrends, skillProfile }) {
...
function chooseRecommendedMode({ recentStats, skillProfile, consistencyScore }) {
  ...
  if (accuracy >= 90) return 'spaced_repetition'
    ...
    messages: [
      { role: 'system', content: INSIGHTS_PROMPT },
      {
        ...
        content: [
          ...
          `Weak topics: ${weakTopics.map((t) => `${t.tag} (${t.accuracy}%)`).join(', ') || 'none yet'}`,
          `Strong topics: ${strongTopics.map((t) => `${t.tag} (${t.accuracy}%)`).join(', ') || 'none yet'}`,
          `Aggregated performance:\n${JSON.stringify(aggregatedData, null, 2)}`,
          studyTrends ? `Study trends:\n${JSON.stringify(studyTrends, null, 2)}` : null,
      ...
      { role: 'system', content: COACH_PROMPT },
      {
        ...
        content: [
          ...
          skillProfile
            ...
            : null,
          'Give direct, specific, motivating exam feedback.',
        ].filter(Boolean).join('\n\n'),
      },
    ],
  })
