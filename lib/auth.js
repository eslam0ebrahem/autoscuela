...
// Token expiry times
const ACCESS_TOKEN_EXPIRY = '365d' // Increased to 365d for persistent sessions like big websites
const REFRESH_TOKEN_EXPIRY = '365d' // Match access token expiry
...
export function setAuthCookie(res, token, rememberMe = true) {
  ...
  if (rememberMe) {
    // Access token cookie matches JWT expiry
    cookieOptions.maxAge = 365 * 24 * 60 * 60 // 365 days
  }
