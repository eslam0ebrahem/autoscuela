...
const requiredEnvVars = ['EMAIL_FROM', 'EMAIL_SMTP_HOST', 'EMAIL_SMTP_PORT', 'EMAIL_SMTP_USER', 'EMAIL_SMTP_PASS']
const missingEnvVars = requiredEnvVars.filter(v => !process.env[v])

if (missingEnvVars.length > 0) {
  console.warn(`[email] Missing environment variables for email: ${missingEnvVars.join(', ')}. Email sending will not work.`)
}
...
function getTransporter() {
  ...
  if (process.env.NODE_ENV === 'development' && (!process.env.EMAIL_SMTP_HOST || process.env.EMAIL_SMTP_HOST === 'localhost')) {
